...
\begin{algorithm}[H]
     ...
     \For {\(i = 1\) to \(n\)}
          ...
          \For {\(k = 1\) to \(i-1\)}
               \If{\(P_{sorted}[k].r > P_{sorted}[i].r\) \textbf{and} \(P_{sorted}[k].h > P_{sorted}[i].h\) \textbf{and} \(1 + dp[k] > dp[i]\)} 
               \\ \Comment{compare \(r\) to avoid counting boxes of the same radius}
                    \State \(dp[i] = 1 + dp[k]\)
                    \State \(prev[i] = k\)
               \EndIf
          \EndFor
...
\begin{algorithm}[H]
     ...
     \begin{algorithmic}[1]
          ...
          \State \(i = 1\); \(j = 1\)
          \For {\(k = p\) to \(r\)}
               \If {\(L[i].r \geq R[i].r\)} \Comment{\textbf{modification here}}
                    \State \(P[k] = L[i]\)
                    \State \(i = i + 1\)
               \Else 
                    \State \(P[k] = R[i]\)
