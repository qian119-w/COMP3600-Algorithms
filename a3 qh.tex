...
\paragraph{(b)} Let \(P\) denote the input sequence \([(r_1,h_1), (r_2,h_2), \hdots, (r_n,h_n)]\). We access the \(i^{th}\) element (box size) in \(P\) by \(P[i]\), and access \(r_i\) by \(P[i].r\) and  \(h_i\) by \(P[i].h\)
\begin{algorithm}
     \caption{Longest Strictly Decreasing Sequence(\(P\))}
     \begin{algorithmic}[1]
...
\paragraph{(c).} Time complexity is \(O(n^2)\)
\end{document}