...
\begin{itemize}
     \setlength \itemsep{0em} 
     \item On LDS:\\
     Suppose \(P_i = [(r_1,h_1), (r_2, h_2), \hdots, (r_i, h_i)]\) is the input sequence, then a LDS that ends with the element \((r_i, h_i)\), is built from a LDS that ends with \((r_k, h_k)\) for \(k \in [1, i-1]\) where \(r_k > r_i\) and \(h_k > h_i\). There could be multiple \(k\)'s that form the longest subsequence. As formula, LDS(\(P_i = \text{longest}([(r_i, h_i)], \underset{\substack{k \in [1, i-1] \\ r_k > r_i \land h_k > h_i}}{\text{longest}}(\text{LDS(\(P_k\))} + [(r_i, h_i)])\)) is a longest sequence among \(S_k\) for \textbf{all} \(k \in [1, i-1]\) where
     \begin{equation*}
          S_{k \in [1, i-1]} = 
          \begin{cases}
